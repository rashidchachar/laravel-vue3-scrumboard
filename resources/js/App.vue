<template>
    <div class="container">
        ...
        <draggable v-model="lists" @end="onListDragEnd"  class="row lists-section p-4" group="lists">
            <list v-for="list in lists" :key="list.id" :list="list" @add-card="addCard" @update-list-cards="updateListCards" />
        </draggable>
    </div>
</template>
        ...
        methods: {
            ...
            async addCard(listId, cardTitle) {
                try {
                    ...
                    this.lists[listIndex].cards.push(response.data);
                } catch (error) {
                    console.error('Error adding card:', error);
                }
            },
            updateListCards(toListId, fromListId) {
                let cardsOrder = [];
